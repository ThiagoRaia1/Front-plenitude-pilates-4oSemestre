  ...
  return (
    <div className="w-[1000px] mx-auto  border border-[#ccc] rounded-lg overflow-hidden pb-2.5 shadow-lg">
      ...
      <div className="flex flex-wrap">
        {dayNames.map((day) => (
          <div className="w-[14.28%] p-2.5 border-2 border-[#f0f0f0] box-border text-center text-xs opacity-70 rounded-sm bg-[#f0f0f0] mb-2" key={day}>{day}</div>
        ))}
      </div>
      ...
      <div className="flex flex-wrap">
        ...
        {daysInMonth.map((day) => (
          <div 
            ...
            className={`border-2 border-[#f0f0f0] cursor-pointer w-[14.28%] p-2.5 box-border text-center text-xs opacity-70 rounded-sm transition-colors duration-300 hover:bg-[#f0f0f0]
 ${
